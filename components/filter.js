...
export default function Filter({ productCount, onSearch, locations }) {
  ...
  const filter = () => {
    ...
    for (let refEl in refEls) {
      if (refEl === "direction") {
        // Handle direction checkbox separately using the direction state
        if (refEls[refEl].current.checked) {
          newQuery += buildQuery(refEl, direction);
        }
      } else {
      ...
      <div className="level-left">
        ...
        <div className="level-item">
          <Input
            ...
            onChangeEvent={() => {
              // Trigger search on each keystroke after a brief delay
              clearTimeout(window.searchTimeout);
              window.searchTimeout = setTimeout(filter, 300);
            }}
            extra={
              <p className="control">
